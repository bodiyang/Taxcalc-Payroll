...
def paramtextdf(df, ptype):
    ...
    def effect_puf_cps_one(row):
        return (
            "_Has An Effect When Using:_"
            + " _PUF data:_ "
            + boolstr(row.compatible_data["puf"])
            + " _CPS data:_ "
            + boolstr(row.compatible_data["cps"])
            + "  \n"
        )
    ...
    def inflation_indexed(df):
        return (
            "_Can Be Inflation Indexed:_ "
            + boolstr(df.indexable)
            + " _Is Inflation Indexed:_ "
            + boolstr(df.indexed)
            + "  \n"
        )

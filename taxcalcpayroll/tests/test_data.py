...
def test_recs_class(recs_varinfo_file, cps_subsample):
    ...
    with pytest.raises(ValueError):
        Recs(data=list(), start_year=2000, gfactors=None, weights=None)
    with pytest.raises(ValueError):
        Recs(data=cps_subsample, start_year=list(), gfactors=None, weights=None)
    with pytest.raises(ValueError):
        Recs(data=cps_subsample, start_year=2000, gfactors=None, weights="")
    with pytest.raises(ValueError):
        Recs(data=cps_subsample, start_year=2000, gfactors=GrowFactors(), weights=None)
    with pytest.raises(ValueError):
        Recs(data=cps_subsample, start_year=2000, gfactors="", weights="")
    # test Recs class for correct instantiation with no aging of data
    syr = 2014
    rec = Recs(data=cps_subsample, start_year=syr, gfactors=None, weights=None)
    ...
    rec = Recs(
        data=cps_subsample, start_year=syr, gfactors=GrowFactors(), weights=wghts_df
    )
    assert isinstance(rec, Recs)
    assert np.all(rec.MARS != 0)
    assert rec.data_year == syr

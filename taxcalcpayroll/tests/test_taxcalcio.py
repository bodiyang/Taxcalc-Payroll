...
@pytest.mark.parametrize(
    "input_data, baseline, reform, assump, outdir",
    [
        (
            "no-dot-csv-filename",
...
def test_init_errors(
    reformfile0, errorreformfile, errorassumpfile, year, base, ref, asm
):
    """
    ...
    tcio = TaxCalcIO(
        input_data=recdf, tax_year=year, baseline=baseline, reform=reform, assump=assump
    )
...
@pytest.mark.parametrize(
    "dumpvar_str, str_valid, num_vars",
    [
        (
            """
    ...
    """,
            True,
            8,
        ),  # these 6 parameters plus added RECID and FLPDYR
        (
            """
...
def test_custom_dump_variables(dumpvar_str, str_valid, num_vars):
    ...
    year = 2018
    tcio = TaxCalcIO(
        input_data=recdf, tax_year=year, baseline=None, reform=None, assump=None
    )
    assert not tcio.errmsg
    tcio.init(
        input_data=recdf,
...
def test_output_options(reformfile1, assumpfile1):
    ...
    try:
        tcio.analyze(
            writing_output_file=True, dump_varset=set(["combined"]), output_dump=True
        )
    except Exception:  # pylint: disable=broad-except
        if os.path.isfile(outfilepath):
            try:
...
def test_no_tables_or_graphs(reformfile1):
    ...
    tcio.analyze(writing_output_file=False, output_tables=True, output_graphs=True)

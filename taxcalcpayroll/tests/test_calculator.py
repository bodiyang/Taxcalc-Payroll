...
def test_make_calculator(cps_subsample):
    ...
    calc = Calculator(policy=pol, records=rec, consumption=consump, verbose=True)
...
def test_make_calculator_with_policy_reform(cps_subsample):
    ...
    assert np.allclose(
        calc.policy_param("_II_em"), np.array([4000] * Policy.DEFAULT_NUM_YEARS)
    )
    exp_STD_Aged = [[1600, 1300, 1300, 1600, 1600]] * Policy.DEFAULT_NUM_YEARS
    assert np.allclose(calc.policy_param("_STD_Aged"), np.array(exp_STD_Aged))
    assert np.allclose(
        calc.policy_param("STD_Aged"), np.array([1600, 1300, 1300, 1600, 1600])
    )
...
def test_make_calculator_with_multiyear_reform(cps_subsample):
    ...
    assert np.allclose(
        calc.policy_param("STD_Aged"), np.array([1600, 1300, 1600, 1300, 1600])
    )
...
def test_calculator_mtr(cps_subsample):
    ...
    (mtr_ptx, mtr_itx, mtr_cmb) = calc.mtr(
        variable_str="e00200p", zero_out_calculated_vars=True
    )
    ...
    (_, _, mtr_combined) = calc.mtr(
        variable_str="e00200s", calc_all_already_called=True
    )
    assert isinstance(mtr_combined, np.ndarray)
    (_, _, mtr_combined) = calc.mtr(
        variable_str="e00650", negative_finite_diff=True, calc_all_already_called=True
    )
    assert isinstance(mtr_combined, np.ndarray)
    (_, _, mtr_combined) = calc.mtr(
        variable_str="e00900p", calc_all_already_called=True
    )
    assert isinstance(mtr_combined, np.ndarray)
    (_, _, mtr_combined) = calc.mtr(variable_str="e01700", calc_all_already_called=True)
    assert isinstance(mtr_combined, np.ndarray)
    (_, _, mtr_combined) = calc.mtr(variable_str="e26270", calc_all_already_called=True)
    assert isinstance(mtr_combined, np.ndarray)
    (_, _, mtr_combined) = calc.mtr(
        variable_str="k1bx14p", calc_all_already_called=True
    )
    assert isinstance(mtr_combined, np.ndarray)
    (_, _, mtr_combined) = calc.mtr(
        variable_str="e00200p", calc_all_already_called=True
    )
...
def test_make_calculator_increment_years_first(cps_subsample):
    ...
    exp_STD_Aged = np.array(
        [
            [1500, 1200, 1200, 1500, 1200],
...
def test_ID_StateLocal_HC_vs_CRT(cps_subsample):
    ...
    assert np.allclose(hc_calc.array("payrolltax"), crt_calc.array("payrolltax"))
...
def test_calculator_using_nonstd_input():
    ...
    calc = Calculator(
        policy=pol, records=nonstd, sync_years=False
...
def test_distribution_tables(cps_subsample):
    ...
    reform = {"UBI_u18": {2014: 1000}, "UBI_1820": {2014: 1000}, "UBI_21": {2014: 1000}}
    pol.implement_reform(reform)
...
def test_mtr_graph(cps_subsample):
    ...
    calc = Calculator(policy=Policy(), records=recs)
    fig = calc.mtr_graph(
        calc, mars=2, income_measure="wages", mtr_measure="ptax", pop_quantiles=False
    )
    assert fig
    fig = calc.mtr_graph(
        calc, income_measure="agi", mtr_measure="itax", pop_quantiles=True
    )
    assert fig
...
def test_itemded_component_amounts(year, cvname, hcname, puf_fullsample):
    ...
    assert np.allclose(calc1.array("standard"), 0.0)
    assert np.allclose(calc2.array("standard"), 0.0)
...
def test_qbid_calculation():
    ...
    recs = Records(data=tpc_df, start_year=TPC_YEAR, gfactors=None, weights=None)
    calc = Calculator(policy=Policy(), records=recs)
...
def test_calc_all_benefits_amounts(cps_subsample):
    ...
    # UBI
    ubi_diff = (calc_ubi.weighted_total("ubi") - calc_base.weighted_total("ubi")) / 1e9
    benefit_cost_diff = (
        calc_ubi.weighted_total("benefit_cost_total")
        - calc_base.weighted_total("benefit_cost_total")
    ) / 1e9
    benefit_value_diff = (
        calc_ubi.weighted_total("benefit_cost_total")
        - calc_base.weighted_total("benefit_cost_total")
...
def test_cg_top_rate():
    ...
    mtr_base = calc_base.mtr(
        variable_str="p23250", calc_all_already_called=True, wrt_full_compensation=False
    )

...
def test_constant_inflation_rate_with_reform():
    ...
    ryr = fyr - 1
    reform = {
        "II_em": {(ryr - 3): 1000, ryr: 20000}  # to avoid divide-by-zero under TCJA
    }
...
def test_variable_inflation_rate_with_reform():
    ...
    reform = {"II_em": {2018: 1000, 2020: 20000}}  # to avoid divide-by-zero under TCJA
    pol.implement_reform(reform)
    pol.set_year(2020)
...
def test_multi_year_reform():
    ...
    reform = {
        "SS_Earnings_c": {2016: 300000, 2017: 500000, 2019: 700000},
        "SS_Earnings_c-indexed": {2017: False, 2019: True},
        "CTC_c": {2015: 2000},
        "EITC_c": {2016: [900, 5000, 8000, 9000], 2019: [1200, 7000, 10000, 12000]},
        "II_em": {2016: 7000, 2019: 9000},
    }
...
def check_eitc_c(ppo, reform, ifactor):
    ...
    for i in range(0, ppo.num_years):
        actual[ppo.start_year + i] = arr[i]
    assert np.allclose(actual[2013], [487, 3250, 5372, 6044], atol=0.01, rtol=0.0)
    assert np.allclose(actual[2014], [496, 3305, 5460, 6143], atol=0.01, rtol=0.0)
    assert np.allclose(actual[2015], [503, 3359, 5548, 6242], atol=0.01, rtol=0.0)
...
def test_reform_makes_no_changes_before_year():
    ...
    ppo.set_year(2015)
    assert np.allclose(
        ppo._II_em[:3], np.array([3900, 3950, 4400]), atol=0.01, rtol=0.0
    )
...
def test_order_of_indexing_and_level_reforms():
    ...
    reform = [
        {"SS_Earnings_c": {2015: 500000}, "SS_Earnings_c-indexed": {2015: False}},
        # now reverse the order of the two reform provisions
        {"SS_Earnings_c-indexed": {2015: False}, "SS_Earnings_c": {2015: 500000}},
    ]
...
def test_section_titles(tests_path):
    ...
    ided_ceiling_pct = (
        "Ceiling On The Benefit Of Itemized Deductions "
    ...
    valid_dict = {
        "": {"": 0},  # empty section_1 implies parameter not displayed in Tax-Brain
        "Parameter Indexing": {"Offsets": 0},
        "Payroll Taxes": {
        ...
        "Other Taxes": {"Net Investment Income Tax": 0},
        "Refundable Credits": {
        ...
        "Surtaxes": {"New Minimum Tax": 0, "New AGI Surtax": 0, "Lump-Sum Tax": 0},
        "Universal Basic Income": {"UBI Benefits": 0, "UBI Taxability": 0},
        "Benefits": {
            "Benefit Repeal": 0,
        },
    }
    ...
    path = os.path.join(tests_path, "..", "..", "docs", "guide", "policy_params.md")
...
def test_reform_with_bad_ctc_levels():
    ...
    """
    pol = Policy()
    child_credit_reform = {"CTC_c": {2020: 2200}, "ACTC_c": {2020: 2500}}
    with pytest.raises(pt.ValidationError):
...
def test_reform_with_warning():
    ...
    """
    exp_warnings = {"ID_Medical_frt": ["ID_Medical_frt[year=2020] 0.05 < min 0.075 "]}
    pol = Policy()
...
def test_index_offset_reform():
    ...
    indexrate1 = pvalue1[2021] / pvalue1[2020] - 1.0
    syear = Policy.JSON_START_YEAR
    expindexrate = cpiu_rates[2020 - syear] + offset
    expvalue = round(pvalue2[2020] * (1.0 + expindexrate), 2)
...
def test_cpi_offset_affect_on_prior_years():
    ...
    np.testing.assert_allclose(
        p1_rates[: 2022 - start_year], p2_rates[: 2022 - start_year]
    )

    # Inflation rate in 2022 was updated.
    np.testing.assert_allclose(
        p1_rates[2022 - start_year], p2_rates[2022 - start_year] - (-0.005)
    )
...
def test_cpi_offset_on_reverting_params():
    ...
    reform2 = {
        "STD": {2020: [10000, 20000, 10000, 10000, 20000]},
        "parameter_indexing_CPI_offset": {2020: -0.001},
    }
    ...
    assert np.allclose(p0._STD[ryear - syear], p1._STD[ryear - syear], atol=0.5)

    # STD was not reverted in 2026 if included in revision
    assert not np.allclose(p1._STD[ryear - syear], p2._STD[ryear - syear], atol=0.5)
...
class TestAdjust:
    ...
    def test_simple_adj(self):
        ...
        exp = np.vstack(
            [
                ...
                (val2020 * (1 + pol2.inflation_rates(year=2020))).round(2)
    ...
    def test_activate_index(self):
        ...
        """
        pol1 = Policy()
        pol1.implement_reform({"CTC_c": {2022: 2000}, "CTC_c-indexed": {2022: True}})
        ...
        exp = np.array(
            [pol0.CTC_c[0], 2000, 2000 * (1 + pol2.inflation_rates(year=2022))]
        ).round(2)
    ...
    def test_apply_cpi_offset(self):
        ...
        pol2 = Policy()
        pol2.adjust(
            {"parameter_indexing_CPI_offset": [{"year": 2021, "value": -0.001}]}
        )
    ...
    def test_multiple_cpi_swaps(self):
        ...
        np.testing.assert_equal(pol2.II_em[0], pol2.II_em[1])
    ...
    def test_multiple_cpi_swaps2(self):
        ...
        pol2.set_state(year=[2018, 2019])
        np.testing.assert_equal(pol2.SS_Earnings_c[0], pol2.SS_Earnings_c[1])
        ...
        for i in (1, 2, 3):
            np.testing.assert_equal(pol2.AMT_em[0], pol2.AMT_em[i])
    ...
    def test_adj_related_parameters_and_index_status(self):
        ...
        res = (pol.sel["II_brk6"]["MARS"] == "single") & (
            pol.sel["II_brk6"]["year"] == 2020
        )
        assert res.isel[0]["value"] == [316700]
        res = (pol.sel["II_brk7"]["MARS"] == "single") & (

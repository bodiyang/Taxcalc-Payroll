...
def test_validity_of_consumption_vars_set():
    ...
    useable_vars = set(
        ["housing", "snap", "tanf", "vet", "wic", "mcare", "mcaid", "other"]
    )
...
def test_update_consumption():
    ...
    revision = {
        "MPC_e20400": {2014: 0.05, 2015: 0.06},
        "BEN_mcare_value": {2014: 0.75, 2015: 0.80},
    }
    ...
    assert np.allclose(
        consump._MPC_e17500, np.zeros((Consumption.DEFAULT_NUM_YEARS,)), rtol=0.0
    )
    ...
    assert np.allclose(consump._BEN_mcare_value, expected_ben_mcare_value, rtol=0.0)
    assert np.allclose(
        consump._BEN_snap_value, np.ones((Consumption.DEFAULT_NUM_YEARS,)), rtol=0.0
    )
...
def test_consumption_response(cps_subsample):
    ...
    (mtr0_ptax, mtr0_itax, _) = calc0.mtr(
        variable_str="e00200p", wrt_full_compensation=False
    )
    ...
    mtr1_ptax, mtr1_itax, _ = calc1.mtr(
        variable_str="e00200p", wrt_full_compensation=False
    )
    ...
    assert np.all(
        np.less_equal(
            np.around(mtr1_itax, decimals=5), np.around(mtr0_itax, decimals=5)
        )
    )

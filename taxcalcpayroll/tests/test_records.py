...
def test_incorrect_Records_instantiation(cps_subsample):
    ...
    with pytest.raises(ValueError):
        recs = Records(
            data=cps_subsample, gfactors=None, weights=None, start_year=list()
        )
    with pytest.raises(ValueError):
        recs = Records(
            data=cps_subsample, gfactors=None, weights=None, adjust_ratios=list()
        )
    ...
    for iotype in ["read", "calc"]:
        for vname in allvars[iotype]:
            ...
            if not indefinite_yrange and len(yranges) > 0:
                prior_ey_ok = (
                    prior_eyr == last_form_year or prior_eyr == last_form_year - 1
                )
                if not prior_ey_ok:
...
def test_csv_input_vars_md_contents(tests_path):
    ...
    civ_path = os.path.join(tests_path, "..", "validation", "CSV_INPUT_VARS.md")
    civ_set = set()

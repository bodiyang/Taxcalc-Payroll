...
def test_round_trip_reforms(fyear, tests_path):
    ...
    for pname in clp_mdata.keys():
        ...
        if not np.allclose(rtr_val, clp_val):
            ...
            msg += "\n  {} in {} : rtr={} clp={}".format(pname, fyear, rtr_val, clp_val)
            if fail_dump:
...
def test_reform_json_and_output(tests_path):
    ...
    def write_res_file(calc, resfilename):
        ...
        """
        varlist = ["RECID", "c00100", "standard", "c04800", "iitax", "payrolltax"]
...
def test_reforms(
    rid, test_reforms_init, tests_path, baseline_2017_law, reforms_dict, puf_subsample
):
    """
    ...
    actual = reform_results(
        rid, reforms_dict[str(rid)], puf_subsample, baseline_2017_law
    )
    afile_path = os.path.join(tests_path, "reform_actual_{}.csv".format(rid))
